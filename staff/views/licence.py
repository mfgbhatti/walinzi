...
def StaffLicenceView(request, staff_id):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            result = sia_search.submit(licence_no=licence_no)
            my_list = (
                "registered",
                "expiry_date",
            )
            for value in my_list:
                date_str = result.get(value)

                try:
                    date_value = datetime.strptime(date_str, "%d %B %Y").date()
                except ValueError:
                    date_value = None

                if date_value is not None:
                    result[value] = date_value.strftime("%Y-%m-%d")
