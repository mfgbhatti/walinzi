    ...
    if request.method == "POST":
        if shift_form.is_valid():
            ...
            for i in range(delta.days + 1):
                ...
                # Create Shift
                # shift = Shift.objects.create(site_id=site_id, time_in=datetime.combine(date, time_in), time_out=datetime.combine(date, time_out))
                shift = Shift.objects.create(site_id=site_id, time_in=timezone.datetime.combine(date, time_in), time_out=timezone.datetime.combine(date, time_out))
            ...
            print("validation faild!") # errors time_in, time_out Enter a valid date/time.
    # for time input
    hours = range(24)
    minutes = ["00", "15", "30", "45"]
    context.update(
